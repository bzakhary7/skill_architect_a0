...
- Create a few test prompts and run skill_architect-with-access-to-the-skill on them
...
1. What should this skill enable SkillArchitect to do end-to-end?
...
└── references/
    ...
    └── azure.md
```
SkillArchitect reads only the relevant reference file.
...
Put results in `<skill-name>-workspace/` next to the staged skill directory in the system temp area (for example: Unix/macOS `/tmp/skill_architect-skills/<skill-name>-workspace/`; Windows `%TEMP%\\skill_architect-skills\\<skill-name>-workspace\\`). Within the workspace, organize results by iteration (`iteration-1/`, `iteration-2/`, etc.) and within that, each test case gets a directory (`eval-0/`, `eval-1/`, etc.). Don't create all of this upfront — just create directories as you go.
...
The description field in SKILL.md frontmatter is the primary mechanism that determines whether SkillArchitect invokes a skill. After creating or improving a skill, offer to optimize the description for better triggering accuracy.
...
This handles the full optimization loop automatically. It splits the eval set into 60% train and 40% held-out test, evaluates the current description (running each query 3 times to get a reliable trigger rate), then calls SkillArchitect to propose improvements based on what failed. It re-evaluates each new description on both train and test, iterating up to 5 times. When it's done, it returns JSON with `best_description` — selected by test score rather than train score to avoid overfitting.

### How skill triggering works

Understanding the triggering mechanism helps design better eval queries. Skills appear in SkillArchitect's `available_skills` list with their name + description, and SkillArchitect decides whether to consult a skill based on that description. The important thing to know is that SkillArchitect only consults skills for tasks it can't easily handle on its own — simple, one-step queries like "read this PDF" may not trigger a skill even if the description matches perfectly, because SkillArchitect can handle them directly with basic tools. Complex, multi-step, or specialized queries reliably trigger skills when the description matches.

This means your eval queries should be substantive enough that SkillArchitect would actually benefit from consulting a skill. Simple queries like "read file X" are poor test cases — they won't trigger skills regardless of description quality.
...
- **Project-level**: `.skill_architect/skills/<skill-name>/SKILL.md` — available only in this project
- **Global**: `~/.config/skill_architect/skills/<skill-name>/SKILL.md` — available in all projects
...
- Run skill_architect-with-access-to-the-skill on test prompts
